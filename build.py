...
def supervisor():
    ...
    conf.set(section_unix_http_server, "file", os.path.join(root, "sdt.sock"))
    conf.set(section_unix_http_server, "username", "sdt")
    conf.set(section_unix_http_server, "password", "sdt2017")
    ...
    conf.set(section_supervisord, "pidfile", "sdt.pid")
    conf.set(section_supervisord, "directory", root)

    section_supervisorctl = "supervisorctl"
    conf.add_section(section_supervisorctl)
    conf.set(section_supervisorctl, "serverurl", "unix://" + os.path.join(root, "sdt.sock"))
    conf.set(section_supervisorctl, "username", "sdt")
    conf.set(section_supervisorctl, "password", "sdt2017")
